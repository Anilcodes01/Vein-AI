...
const ProgressCircle: React.FC<ProgressCircleProps> = ({ value, maxValue, label, unit, color }) => {
  const percentage = maxValue > 0 ? Math.min(Math.round((value / maxValue) * 100), 100) : 0;
  const radius = 35;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (percentage / 100) * circumference;
  
  // Define gradient IDs based on nutrition type
  const gradientMap = {
  ...
  const gradientId = gradientMap[color as keyof typeof gradientMap] || "default-gradient";
  
  // Get icon based on label
  const getIcon = () => {
    switch(label.toLowerCase()) {
      case 'calories': return <Flame className="w-4 h-4" />;
      case 'protein': return <TbMeat className="w-4 h-4" />;
      case 'carbs': return <Cookie className="w-4 h-4" />;
      case 'fat': return <PiGrains className="w-4 h-4" />;
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="mt-2 flex flex-col items-center">
        ...
        <div className="text-sm mt-1">
          ...
          <span className="text-base-content/60">/{maxValue} {unit}</span>
        </div>
      </div>
    </div>
  );
